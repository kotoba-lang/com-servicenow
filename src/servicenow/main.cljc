(ns servicenow.main
  "Kotodama WASM entrypoint for the Servicenow clean-room actor (L4) — Clojure port.
  ...
  facts (`servicenow.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "servicenow")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"servicenow.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "servicenow-compat" :tier tier :entities entities} 200])
...
(defn start! [] :servicenow-compat/ready)
